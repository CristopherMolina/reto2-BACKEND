...
def biografia():
    if ('token' in session):
        biografiaData = fb.getDocument('biografia','lYTHw9dZhosLuWQz9l2C')
        print(biografiaData)
        ...
        if biografia_form.validate_on_submit():
            ...
            resultadoUpdateBiografia = fb.updateDocument('biografia','lYTHw9dZhosLuWQz9l2C',dataBiografiaActualizada)
...
def proyectos():
    if('token' in session):
        listaProyectos = fb.getCollection('proyecto')
        ...
        if proyecto_form.validate_on_submit():
            ...
            nuevoProyecto = fb.insertDocument('proyecto',dataNuevoProyecto)
            print(nuevoProyecto)
...
def proyecto(id=''):
    if('token' in session):
        listaProyectos = fb.getCollection('proyecto')
        proyectoData = fb.getDocument('proyecto',id)
        print(proyectoData)
        ...
        if proyecto_form.validate_on_submit():
            ...
            resultadoActualizarProyecto = fb.updateDocument('proyecto',id,dataProyectoActualizar)
